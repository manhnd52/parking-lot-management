...
public class ParkingHistoryDAO implements DAOInterface<ParkingHistory> {
	...
	public List<ParkingHistory> getByStudentId(String studentId) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<ParkingHistory> query = session.createQuery("FROM ParkingHistory t WHERE t.student.student_id=:id",
	...
	public ParkingLot getCurrentPosition(Student student) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<ParkingLot> query = session.createQuery(
					"SELECT t.parking_lot FROM ParkingHistory t " + "WHERE t.student=:student AND t.exit_time = NULL",
					ParkingLot.class);
